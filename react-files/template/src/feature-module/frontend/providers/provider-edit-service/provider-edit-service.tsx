...
const ProviderEditService = () => {
  ...
  useEffect(() => {
    const fetchServiceDetails = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          setPhoneNumber(service.mobile_number);
          setAddress(service.location_base);
          setRadius(service.location_radius);
          setVehicleReg(service.vehicle_reg);
          setselectedVehicleSize(service.van_size);
      ...
      try {
        const availabilityResponse = await axios.get(
          `https://wgjgzlvwmoavtpeylund.supabase.co/rest/v1/availability?driver_service_id=eq.${serviceId}`,
          {
            headers: {
              Authorization: `Bearer ${access_token}`,
              apiKey: process.env.REACT_APP_BEARER_TOKEN,
        ...
        if (availabilityResponse.status === 200) {
          ...
          // Update the state variables with the availability data
          availability.forEach((day) => {
            switch (day.day_of_week) {
              case 'monday':
                setMonday({ start: day.start_time, end: day.end_time });
                break;
              case 'tuesday':
                setTuesday({ start: day.start_time, end: day.end_time });
                break;
              case 'wednesday':
                setWednesday({ start: day.start_time, end: day.end_time });
                break;
              case 'thursday':
                setThursday({ start: day.start_time, end: day.end_time });
                break;
              case 'friday':
                setFriday({ start: day.start_time, end: day.end_time });
                break;
              case 'saturday':
                setSaturday({ start: day.start_time, end: day.end_time });
                break;
              case 'sunday':
                setSunday({ start: day.start_time, end: day.end_time });
                break;
              default:
                break;
            }
      ...
      try {
        const pricesResponse = await axios.get(
          `https://wgjgzlvwmoavtpeylund.supabase.co/rest/v1/pricing?driver_service_id=eq.${serviceId}`,
          {
            headers: {
              Authorization: `Bearer ${access_token}`,
              apiKey: process.env.REACT_APP_BEARER_TOKEN,
        ...
        if (pricesResponse.status === 200) {
          // Update the prices state variable with the prices data
          const initialPrices = {};
          pricesResponse.data.forEach((price) => {
            const key = `${price.van_size}_${price.helper_count}_${price.day}`;
            initialPrices[key] = price.price;
          });
      ...
      try {
        const mileageResponse = await axios.get(
          `https://wgjgzlvwmoavtpeylund.supabase.co/rest/v1/mileage_pricing?driver_service_id=eq.${serviceId}`,
          {
            headers: {
              Authorization: `Bearer ${access_token}`,
              apiKey: process.env.REACT_APP_BEARER_TOKEN,
        ...
        if (mileageResponse.status === 200 && mileageResponse.data.length > 0) {
          // Update the mileage state variable with the mileage data
          setLongDistancePrices(mileageResponse.data[0].price);
        }
      } catch (error) {
        console.error('Error:', error);
      }
      ...
      try {
        const floorPricesResponse = await axios.get(
          `https://wgjgzlvwmoavtpeylund.supabase.co/rest/v1/floors_pricing?driver_service_id=eq.${serviceId}`,
          {
            headers: {
              Authorization: `Bearer ${access_token}`,
              apiKey: process.env.REACT_APP_BEARER_TOKEN,
        ...
        if (floorPricesResponse.status === 200) {
          // Update the floorPrices state variable with the floor prices data
          const initialFloorPrices = {};
          floorPricesResponse.data.forEach((price) => {
            initialFloorPrices[price.helpers] = price.price;
          });
          setFloorPrices(initialFloorPrices);
        }
      } catch (error) {
        console.error('Error:', error);
      }
      ...
      try {
        const packingPricesResponse = await axios.get(
          `https://wgjgzlvwmoavtpeylund.supabase.co/rest/v1/packing_pricing?driver_service_id=eq.${serviceId}`,
          {
            headers: {
              Authorization: `Bearer ${access_token}`,
              apiKey: process.env.REACT_APP_BEARER_TOKEN,
        ...
        if (
          packingPricesResponse.status === 200 &&
          packingPricesResponse.data.length > 0
        ) {
          // Update the packingPrices state variable with the packing prices data
          const initialPackingPrices = {};
          packingPricesResponse.data.forEach((price) => {
            initialPackingPrices[price.packing] = price.price;
          });
          setPackingPrices(initialPackingPrices);
          setIsPacking(true);
        }
      } catch (error) {
        console.error('Error:', error);
      ...
      try {
        const assemblyPricesResponse = await axios.get(
          `https://wgjgzlvwmoavtpeylund.supabase.co/rest/v1/assembly_pricing?driver_service_id=eq.${serviceId}`,
          {
            headers: {
              Authorization: `Bearer ${access_token}`,
              apiKey: process.env.REACT_APP_BEARER_TOKEN,
        ...
        if (
          assemblyPricesResponse.status === 200 &&
          assemblyPricesResponse.data.length > 0
        ) {
          // Update the assemblyPrices state variable with the assembly prices data
          const initialAssemblyPrices = {};
          assemblyPricesResponse.data.forEach((price) => {
            initialAssemblyPrices[price.assembly] = price.price;
          });
          setAssemblyPrices(initialAssemblyPrices);
          setIsAssembly(true);
        }
      } catch (error) {
        console.error('Error:', error);
      ...
      try {
        const pianoPricesResponse = await axios.get(
          `https://wgjgzlvwmoavtpeylund.supabase.co/rest/v1/piano_pricing?driver_service_id=eq.${serviceId}`,
          {
            headers: {
              Authorization: `Bearer ${access_token}`,
              apiKey: process.env.REACT_APP_BEARER_TOKEN,
        ...
        if (
          pianoPricesResponse.status === 200 &&
          pianoPricesResponse.data.length > 0
        ) {
          // Update the pianoPrices state variable with the piano prices data
          const initialPianoPrices = {};
          pianoPricesResponse.data.forEach((price) => {
            const pianoType =
              price.piano === 'upright' ? 'Upright Piano' : 'Grand Piano';
            initialPianoPrices[pianoType] = price.price;
          });
          setPainoPrices(initialPianoPrices);
          setIsPiano(true);
        }
      } catch (error) {
        console.error('Error:', error);
    ...
    fetchServiceDetails();
  }, [serviceId]); // Depend on serviceId so it
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Call the API without checking if any state has changed
      try {
      ...
      const days = [
        { name: 'monday', state: monday },
        { name: 'tuesday', state: tuesday },
        { name: 'wednesday', state: wednesday },
        { name: 'thursday', state: thursday },
        { name: 'friday', state: friday },
        { name: 'saturday', state: saturday },
        { name: 'sunday', state: sunday },
      ];

      const workingHoursParams = days.map((day) => ({
        day_of_week: day.name,
        start_time: day.state.start,
        end_time: day.state.end,
      }));

      // Call the PATCH API without checking if any day has changed
      try {
        const workingHoursResponse = await axios.patch(
          `https://wgjgzlvwmoavtpeylund.supabase.co/rest/v1/availability?id=eq.${serviceId}`,
          workingHoursParams,
          {
            headers: {
              Authorization: `Bearer ${access_token}`,
        ...
        for (const [index, image] of images.entries()) {
          ...
          try {
            const docResponse = await axios.post(
              `https://wgjgzlvwmoavtpeylund.supabase.co/storage/v1/object/documents/${user_id}/${serviceId}/${section}/${index}`,
              formData,
              {
                headers: {
                  Authorization: `Bearer ${access_token}`,
            ...
            <div className="container">
              <div className="row">
                <div className="col-md-12 col-12">
                  <h2 className="breadcrumb-title">Edit Service</h2>
                </div>
              </div>
            </div>
          </div>
                                        ...
                                        <div className="col-lg-6 pricing-planscontent pricing-planscontent-five">
                                          ...
                                          <div className="form-group">
                                            <input
                                              ...
                                              value={
                                                prices[
                                                  `${vehicleSize}_${loadingOption}_weekend`
                                                ]
                                              }
                                              onChange={(e) =>
                                                handlePriceChange(
                                ...
                                <div className="col-lg-12 pricing-planscontent pricing-planscontent-five">
                                  ...
                                  <div className="form-group">
                                    <input
                                      ...
                                      className="form-control"
                                      placeholder="0"
                                      value={floorPrices[floorHelper] || 0}
                                      onChange={(e) =>
