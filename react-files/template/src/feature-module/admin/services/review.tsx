...
import { ReviewInterface } from '../../../core/models/interface';

const Review = () => {
  const data = useSelector((state:ReviewInterface) => state.review_data);

  const [selectedValue, setSelectedValue] = useState(null);
  const value = [{ name: 'A - Z' }, { name: 'Z - A' }];

  const providerName = (data: ReviewInterface) => {
    return (
      <div>
  ...
  const userName = (data: ReviewInterface) => {
    return (
      <div>
        <Link to="#" className="table-profileimage">
  ...
  const serviceName = (data: ReviewInterface) => {
    return (
      <div>
